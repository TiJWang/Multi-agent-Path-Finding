...
def check_goal_reached(init_pose, goal_pose, bias):
    if (
            init_pose.pose.position.x > goal_pose.pose.position.x - bias and init_pose.pose.position.x < goal_pose.pose.position.x + bias \
...
goal_pose1 = rospy.wait_for_message('/id101/aruco_single/pose', PoseStamped)
goal_pose2 = rospy.wait_for_message('/id100/aruco_single/pose', PoseStamped)
goal_poses = [goal_pose1, goal_pose2]
twist = Twist()
...
for goal_pose in goal_poses:
    ...
    if theta < 0 and abs(theta) > abs(theta + 2 * math.pi):
        ...
        heta = theta - 2 * math.pi
    current_pos = "".join([str(init_pose.pose.position.x), str(init_pose.pose.position.y)])
